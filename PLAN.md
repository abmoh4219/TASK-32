...
- [x] 4.4 Create backend/src/handlers/outcomes.rs — POST /api/outcomes (returns duplicate_candidates), POST /api/outcomes/:id/contributors, DELETE /api/outcomes/:id/contributors/:id, POST /api/outcomes/:id/evidence (multipart), POST /api/outcomes/:id/submit, GET /api/outcomes/:id/compare/:other_id (side-by-side diff)
- [x] 4.5 Register outcome routes: Reviewer + Administrator roles for write operations
- [x] 4.6 Create frontend/src/pages/outcomes/register.rs — multi-step form: Step 1 (type select with descriptive cards for paper/patent/competition_result/software_copyright), Step 2 (title + abstract with real-time duplicate check → amber warning banner "X similar outcomes found" + "View Comparison" link if ≥0.85), Step 3 (evidence drag-drop upload, progress bar, MIME error inline), Step 4 (contributor allocation: name search, share % input, running total bar turns green at 100 / red if over or under)
- [x] 4.7 Create frontend/src/pages/outcomes/compare.rs — side-by-side golden split-card layout, fields highlighted amber where different, similarity score displayed at top
- [x] 4.8 Fill in backend/tests/unit_tests/outcome_tests.rs:
...
- [x] 4.9 Fill in backend/tests/api_tests/outcome_api.rs:
...
- [x] 4.10 Fill in frontend/tests/unit_tests/validation_tests.rs:
